...
import { useMasterchef, useSousChef, useSugarswapStrategy } from './useContract'
...
export const useVaultHarvest = (strategyAddress: string) => {
  const dispatch = useDispatch()
  const { account } = useWeb3React()
  const sugarswapStrategyContract = useSugarswapStrategy(strategyAddress)
  const { gasBoostedPrice } = useGasBoost()

  const handleHarvest = useCallback(async () => {
    const txHash = await harvestVault(sugarswapStrategyContract, account, gasBoostedPrice)
    dispatch(fetchVaultUserDataAsync(account))
    return txHash
  }, [account, dispatch, sugarswapStrategyContract, gasBoostedPrice])
