  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      ...
      <Wrap>
        <div>
          <div>
            <Tips>{t(`1 Ticket = %lotteryPrice% SUGAR`, { lotteryPrice: ticketPrice })}</Tips>
          </div>
          <div>
            <Announce>
              {t('Ticket purchases are final. Your SUGAR cannot be returned to you after buying tickets.')}
            </Announce>
          </div>
        </div>
        <div>
          <StyledMaxText>{t(`%num% SUGAR Available`, { num: fullBalance.toLocaleString() })}</StyledMaxText>
        </div>
      </Wrap>
      <Final>
        <FinalContent>{t(`You will spend: %num% SUGAR`, { num: cakeCosts(val).toLocaleString() })}</FinalContent>
      </Final>
      <ModalActions>
