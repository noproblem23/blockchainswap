...
import { Image, Heading, RowType, Toggle, Text } from 'sugarswap-uikit'
import styled from 'styled-components'
import FlexLayout from 'components/layout/Flex'
import Page from 'components/layout/Page'
import { useValuts, usePriceSugarUsd, useGetApiPrices } from 'state/hooks'
  ...
  const vaultsList = useCallback(
    (vaultsToDisplay: Vault[]): VaultWithStakedValue[] => {
      let vaultsToDisplayWithAPY: VaultWithStakedValue[] = vaultsToDisplay.map((vault) => {
        ...
        const rewardPrice = prices.sugar
        ...
        <FlexLayout>
          <Route exact path={`${path}`}>
            {vaultsStaked.map((vault) => (
              <VaultCard key={vault.pid} vault={vault} sugarPrice={sugarPrice} account={account} removed={false} />
            ))}
          </Route>
          <Route exact path={`${path}/history`}>
            {vaultsStaked.map((vault) => (
              <VaultCard key={vault.pid} vault={vault} sugarPrice={sugarPrice} account={account} removed />
            ))}
          </Route>
        </FlexLayout>
      </div>
    ...
    <>
      ...
      <Background>
        <Page>
          {/* {blockNumber !== 0 && blockNumber <= START_TIMESTAMP && (
            <NotifyContainer>
              ...
              <Flex alignItems="center" justifyContent="space-between" mb="8px">
                <Text fontSize="20px">{t(`SUGAR Farming Starts On Block ${START_TIMESTAMP}`)}</Text>
                <Text fontSize="20px">{`${START_TIMESTAMP - blockNumber} blocks left until SUGAR Farming`}</Text>
              </Flex>
            </NotifyContainer>
          )} */}
          <ControlContainer>
          ...
          {renderContent()}
          <StyledImage src="/images/assets/farmjet.svg" alt="Sugarswap illustration" width={120} height={103} />
        </Page>
      </Background>
    </>
  )
