...
import { LinkExternal, Text } from 'sugarswap-uikit'
...
const ActionPanel: React.FunctionComponent<ActionPanelProps> = ({ details, apr, liquidity }) => {
  ...
  const exchangeLink = vault.isSingle
    ? `${EXCHANGE_URL}/#/swap?outputCurrency=${lpAddress}`
    : `${EXCHANGE_URL}/#/add/${liquidityUrlPathParts}`

  const bsc = `https://explorer.zksync.io/address/${lpAddress}`
  const info = `https://info.${MAIN_DOMAIN}/${vault.isSingle ? 'token' : 'pair'}/${lpAddress}`
