...
import { Flex, Text, Skeleton } from 'sugarswap-uikit'
...
const RainsugarLight = keyframes`
	0% {
...
const StyledCardAccent = styled.div`
  ...
  animation: ${RainsugarLight} 2s linear infinite;
  border-radius: 32px;
  filter: blur(6px);
  position: absolute;
  top: -2px;
...
const VaultCard: React.FC<VaultCardProps> = ({ vault, removed, account }) => {
  ...
  const earnLabel = vault.lpSymbol // vault.dual ? vault.dual.earnLabel : 'SUGAR  '
  ...
  return (
    <FCard>
      {['SUGAR'].includes(vault.token.symbol) && <StyledCardAccent />}
      <CardHeading
        lpLabel={lpLabel}
