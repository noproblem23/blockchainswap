...
import { Modal, Flex, Text } from 'sugarswap-uikit'
import { useDispatch } from 'react-redux'
import BigNumber from 'bignumber.js'
import { useTranslation } from 'contexts/Localization'
import { useSugar, usePancakeRabbits, useProfile } from 'hooks/useContract'
...
const ConfirmProfileCreationModal: React.FC<Props> = ({
  ...
  const {
    ...
    onRequiresApproval: async () => {
      try {
        const response = await sugarContract.methods.allowance(account, profileContract.options.address).call()
    ...
    onApprove: () => {
      return sugarContract.methods.approve(profileContract.options.address, allowance.toJSON()).send({ from: account })
    },
    onConfirm: () => {
