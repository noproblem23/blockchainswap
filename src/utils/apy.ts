...
import { SECONDS_PER_YEAR, SUGAR_PER_BLOCK, VAULTS_DISTRIBUTION_PERCENTAGE } from 'config'
 ...
 * @param tokenPerSecond Amount of new sugar allocated to the pool for each new block
...
export const getVaultApy = (
  ...
  const yearlySugarRewardAllocation = SUGAR_PER_BLOCK.times(SECONDS_PER_YEAR).times(poolWeight)
  const _apy = yearlySugarRewardAllocation.times(rewardPriceUSD).div(poolLiquidityUsd).toString(10)
