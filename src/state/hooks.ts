...
import { Toast, toastTypes } from 'sugarswap-uikit'
...
export const usePriceSugarUsd = (): BigNumber => {
  const pid = 1 // SUGAR-CORE LP
  const ethPriceUSD = usePriceEthUsd()
  const farm = useFarmFromPid(pid)
  const price = farm.tokenPriceVsQuote ? ethPriceUSD.times(farm.tokenPriceVsQuote) : ZERO
...
export const useGetApiPrices = () => {
  ...
  const ethPriceUSD = usePriceEthUsd()
  // const { xsugarRatio } = useSugarInfo()
  ...
  return {
    ...
    // wcore: corePriceUSD,
    sugar: sugarPriceUSD,
    // xsugar: sugarPriceUSD.times(xsugarRatio),
    // hunt: huntPriceUSD,
